...
/usr/local/bin/hev-dns-forwarder 0.0.0.0 5300 8.8.8.8 &
sleep 1

# 透明代理服务，iptables 的 REDIRECT target 会将命中的 TCP 连接重定向至此服务，socks5-tproxy封装至 socks5 请求转发至 socks5-client。
/usr/local/bin/hev-socks5-tproxy 0.0.0.0 10800 0.0.0.0 $PROXY_PORT &
sleep 1

# 导入 iptables 规则。
sudo iptables-restore /usr/local/bin/iptables-transparent-bypass.rules
